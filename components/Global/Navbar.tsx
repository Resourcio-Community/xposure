...
export default function Navbar() {
  ...
  const { signInWithGoogle, user, authLoading, logOut } = useAuthContext();
  ...
  return (
    <div className="flex w-full justify-between fixed top-0 px-6 md:px-10 z-50 backdrop-blur-lg">
      ...
      <div className={`gap-7 text-sm font-poppins font-light text-white ${user ? 'pl-0' : 'pl-28'} items-center tracking-wide md:flex hidden`}>
        ...
        {user && (
          <Link href="/submission" prefetch={true} className="hover:text-text_yellow">
            Submission
          </Link>
        )}
      </div>
      {authLoading
        ? (
          <div className="flex items-center">
            <Preloader bgHeight="40px" bgWidth="40px" width="2rem" height="2rem" color="#FFE39C" />
          </div>
        ) : (
          <div className="flex items-center">
            {user ? (
              <div className="relative group">
                ...
                <div className={`absolute  group-hover:scale-100 top-[110%] scale-0 px-1 w-[10rem] flex flex-col items-center bg-text_yellow right-0 duration-300 origin-top-right gap-2 py-4 rounded-lg`}>
                  ...
                  <div className="md:hidden flex flex-col items-center gap-2 mt-4">
                    ...
                    {user && (
                      <Link href="/submission" prefetch={true} className="px-4 py-2 hover:bg-neutral-200 duration-300 w-full text-center" >
                        Submission
                      </Link>
                    )}
                  </div>
